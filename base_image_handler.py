...
class BinarizationMethods(Enum):
    MANUAL = 0
    PERCENT_SELECTION = 1
    OTSU = 2
    NIBLACK = 3
...
class BaseImageHandler:
    ...
    def ask_linear_filter_parameters(self):
        ...
        values = {"Low pass": "0", "Prewitt": "1", "Sobel": "2",
                  "Laplace": "3", "Edge detection": "4", "Gauss": "5", "Custom": "x"}
        ...
        kernels = {
            ...
            "4": {"kernel": np.array([[1, 1, 1], [1, -2, -1], [1, -1, -1]]),
                  "convert_to_grayscale": True},
            "5": {"kernel": (1 / 16) * np.array([[1, 2, 1], [2, 4, 2], [1, 2, 1]]),
                  "convert_to_grayscale": False},
    ...
    def ask_binarization_method(self):
        root = tk.Toplevel()
        root.title("Select binarization method")

        option_var = tk.StringVar(value="0")
        values = {"Manual": "0", "Percent selection": "1", "Otsu": "2", "Niblack": "3"}
        tk.Label(root, text="Select binarization method:").grid(row=0, column=0, columnspan=2, pady=(10, 5))
        i = 1
        for (text, value) in values.items():
            tk.Radiobutton(root, text=text, variable=option_var, value=value).grid(row=i, column=0,
                                                                                   columnspan=2, sticky="w",
        ...
        option_value = int(option_var.get())
        method = BinarizationMethods(option_value)
        return method

    def apply_binarization(self, method):
        match method:
            case BinarizationMethods.MANUAL:
                self.manual_binarization()
            case BinarizationMethods.PERCENT_SELECTION:
                self.percent_selection_binarization()
            case BinarizationMethods.OTSU:
                self.otsu_binarization()
            case BinarizationMethods.NIBLACK:
                window_size = self.ask_niblack_window_size()
                self.niblack_binarization(window_size)

    def manual_binarization(self):
        root = tk.Toplevel()
        root.title("Enter the threshold")
        threshold_var = tk.StringVar(value="127")
        tk.Label(root, text="Enter the threshold").grid(row=0, column=0)
        tk.Entry(root, textvariable=threshold_var).grid(row=0, column=1)
    ...
    def percent_selection_binarization(self):
        root = tk.Toplevel()
        root.title("Enter the percent limit")
        percentage_var = tk.StringVar(value="50")
        tk.Label(root, text="Enter the percent limit (%)").grid(row=0, column=0)
        tk.Entry(root, textvariable=percentage_var).grid(row=0, column=1)
        ...
        def binarize():
            ...
            arr = np.array(self.img_modified).astype(np.uint8)
            all = arr.size
            limes = (percentage / 100) * all
            next_sum = 0
            for i in range(0, 256):
                next_sum = next_sum + len(arr[arr == i])
                if next_sum < limes:
                    lut[i] = 0
                else:
                    lut[i] = 255
    ...
    def otsu_binarization(self):
        ...
        sigma_b_squared = (mu_t * omega - mu) ** 2 / (omega * (1 - omega) + 1e-9)
    ...
    def ask_niblack_window_size(self):
        root = tk.Toplevel()
        root.title("Select the size of the window")

        size_var = tk.StringVar(value="3")
        sizes = ["3", "5", "7", "9"]

        tk.Label(root, text="Select the size of the window:").grid(row=0, column=0, columnspan=2, pady=(10, 5))
    ...
    def niblack_binarization(self, window_size):
        root = tk.Toplevel()
        root.title("Niblack thresholding parameter")
        k_var = tk.StringVar(value="-0.2")
        tk.Label(root, text="Enter k parameter for Niblack:").grid(row=0, column=0)
        tk.Entry(root, textvariable=k_var).grid(row=0, column=1)
